...
namespace Peer.PublicCsharpModule.zip
{
    class CompressHelper
    {
        ...
        //单文件解压
        public Boolean DeCompressSingle(string sourceFilePath,string targetFilePath)
        {
            ...
            try
            {
                ...
                using (FileStream childFileStream = File.Create(targetFilePath))
                {
                    childFileStream.Write(data, 0, data.Length);
                }
                RetFlag = true;
            }
            catch (Exception ex)
            {
               string _errorInfo = Log.LogLib.GetExceptionInfo(ex, "Peer.PublicCsharpModule.zip->CompressHelper->DeCompressSingle", @sourceFilePath);
                Log.LogLib.WriteLog(_errorInfo, Log.LogType.Wrong);
            }
            return RetFlag;
        }
